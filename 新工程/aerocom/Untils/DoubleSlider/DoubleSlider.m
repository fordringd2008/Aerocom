...
- (id) initWithFrame:(CGRect)aFrame minValue:(float)aMinValue maxValue:(float)aMaxValue barHeight:(float)height
{
    ...
    if (self)
	{
		...
		self.minHandle = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"handle"] highlightedImage:[UIImage imageNamed:@"handle_highlight"]];
		self.minHandle.center = CGPointMake(sliderBarWidth * 0.2, sliderBarHeight * 0.5);
        //self.minHandle.frame.size = CGSizeMake(10, 10);
		[self addSubview:self.minHandle];
		
		self.maxHandle = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"handle"] highlightedImage:[UIImage imageNamed:@"handle_highlight"]];
		self.maxHandle.center = CGPointMake(sliderBarWidth * 0.8, sliderBarHeight * 0.5);
		[self addSubview:self.maxHandle];
		
		bgColor = CGColorRetain(RGB(24, 177, 17).CGColor);
...
+ (id) doubleSlider
{
	return [[self alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth - 40, 40) minValue:0.0 maxValue:24.0 barHeight:10.0];
}
