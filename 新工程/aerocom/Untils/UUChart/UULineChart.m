...
-(void)setXLabels:(NSArray *)xLabels 
{
    ...
    _xLabelWidth = (scw- UUYLabelwidth)/(num);
    
    for (int i=0; i<xLabels.count; i++) {
        NSString *labelText = xLabels[i];
        UUChartLabel * label = [[UUChartLabel alloc] initWithFrame:CGRectMake(i * _xLabelWidth+UUYLabelwidth - 10, self.frame.size.height - UULabelHeight, _xLabelWidth + 5, UULabelHeight)];
...
//                 value:firstValue];
        ...
        for (NSString * valueString in childAry)
        {
            ...
            if (index == 0)  // 加入第一个点，防止 只有一个数据的时候，不显示划线 图表中不显示任何东西
            {
                UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
                btn.backgroundColor = (UIColor *)(self.colors[0]);
                btn.frame = CGRectMake(point.x-1, point.y-1, 2, 2);
                [self addSubview:btn];
            }
            else//if (index != 0)
            {
...
//                    [progressline stroke];
                    ...
                    btn.layer.borderColor = ((UIColor *)(self.colors[0])).CGColor;
                    btn.layer.borderWidth = 2;
                    btn.frame = CGRectMake(point.x - 3, point.y - 3, 6, 6);
